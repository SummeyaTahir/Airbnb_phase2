...
function BookingPage() {
  ...
  const { listing } = location.state;  // Extract the listing passed from Details
  ...
  const handleBooking = () => {
    ...
    if (!billingInfo.street || !billingInfo.city || !billingInfo.zip || !billingInfo.country) {
      newErrors.billingInfo = 'Please fill in all billing address fields';
    }

    setErrors(newErrors);

    // If no errors, proceed with booking
    if (!newErrors.checkIn && !newErrors.checkOut && !newErrors.billingInfo) {
      const bookingData = {
      ...
      <div id="main-container">
        <div>
          ...
          <BillingAddress 
            ...
            errors={errors.billingInfo}
          />
           <button id="button" onClick={handleBooking}>Confirm Booking</button>
        </div>
